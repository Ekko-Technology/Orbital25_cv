...
def expand_contour(img, contour, expansion_factor):
    ...
    expanded_crop = cv2.resize(cropped, (int(width * expansion_factor), int(height * expansion_factor)))  # Resize the cropped image to the modified size

    # creates a boolean mask of whether each pixel is 0, this is to remove any excess black pixels added from the black size
...
def display_contours(num_of_changes):

    # img_path = rd.choice(list_of_images)
    # img = cv2.imread(os.path.join("../sample_imgs", img_path))
    img = cv2.imread("lena.jpg")  # Load a sample image for contour detection

    # resize image to intended size for consistency
    img = cv2.resize(img, (640, 640))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # Convert the image from BGR to RGB format for display
    ...
    thresh = cv2.Canny(blurred_img, 30, 100)  # Canny edge detection to find edges in the image
